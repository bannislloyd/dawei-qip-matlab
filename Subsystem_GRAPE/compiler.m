...
for m=1:number   
    if length(uncompiled_pulse{m})==1 % no initial Gaussian guess
        ...
        B=[Bx,By]; % first half of B is Bx, second half is By
        Aeq=zeros(length(B),length(B)); beq=zeros(1,length(B));
        lb=-6000*ones(1,length(B)); ub=6000*ones(1,length(B)); % Important!!! try different ub and lb. Unit: Hz
        ...
        for k=1:7 % iterations for optimization. Can change 7 to other numbers
