...
mm = 1;
for theta = 0:pi/60:pi/4
phi = theta; % for the second rotation
...
% rho_3 = (NOT1*NOT2*Hadamard(2,5)*U2')*rho_2*(NOT1*NOT2*Hadamard(2,5)*U2')';
%% weak value
sigma_y_md = trace(rho_3*2*KIy{3})
sigma_z_md = trace(rho_3*2*KIz{3})
sigma_z_ps = trace(rho_3*2*KIz{4})
S=sigma_y_md/g/(1+sigma_z_ps) 

fprintf(fid, 'theta: pi/60*%d, ',mm-1);
fprintf(fid, '<y3>: %1.3f, ',sigma_y_md);
fprintf(fid, '<z3>: %1.3f, ',sigma_z_md);
fprintf(fid, '<z4>:%1.3f, ',sigma_z_ps);
fprintf(fid, 'S:%1.3f\n',S);
