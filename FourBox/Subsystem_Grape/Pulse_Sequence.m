...
U_ideal{1}= expm(i*4e-6*H_int)*After_U2*expm(i*4e-6*H_int);
...
uncompiled_pulse{1}={[40e-3, 200e-6]}; 

[rf_x, rf_y]=compiler(U_ideal, uncompiled_pulse);
...
total_time=length(Bx)*2e-5; % modify step size 
ref_power=6000; % modify calibration power
Amp=abs(Bx+i*By)*100/ref_power; Phase=angle(Bx+i*By)*180/pi;
fid=fopen('After_U2.txt','w'); % modify pulse name
fprintf(fid, '##TITLE= %s\n','Gaussian');
