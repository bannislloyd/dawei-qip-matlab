...
% U_ideal{1}=rot(KIx{2}+KIx{3}+KIx{6},pi)*rot(-KIy{7},pi/2);
U_ideal{1}=rot(KIx{7},pi/2);
% rho_final = U_ideal{1}*rho_ini*U_ideal{1}';
...
% compile option= 
%   I: eliminate nontrivial post_errors; effective when Initial state is on Z-direction
%  II: eliminate nontrivial pre_errors and post_erros; 
% III: eliminate nontrivial pre_errors;  effective when Final state is on Z-direction
%uncompiled_pulse{1}={[5e-3, 5e-6], [1, 2.5e-3, 5e-3, 0, 180, 300;6, 2.5e-3, 5e-3, 0, 180, 300], {'I','I'}};
%uncompiled_pulse{1}={[2e-3, 2e-6], [2, 1e-3, 2e-3, 0, 180, 300;3, 1e-3, 2e-3, 0, 180, 300;6, 1e-3, 2e-3, 0, 180, 300;7, 1e-3, 2e-3, 0, 90, 300], {'I','I','I','III'}};
uncompiled_pulse{1}={[1e-3, 1e-6], [7, 0.5e-3, 1e-3, 0, 90, 300], {'I'}};
