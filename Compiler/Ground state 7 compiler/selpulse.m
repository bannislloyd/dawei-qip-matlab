...
% this function is used to get Bx and By through reading the parameters of pulse;
...
for k=1:size(pulse_description{2},1) % size: "1" returns the number of rows; "2" returns the number of columns;
    ...
    % shape of Gaussian 
    Gaussian=exp(-((1:M)-M/2).*((1:M)-M/2)/(pulse_variance)^2); 
    % normalization by the angle of rotating: sum(2*pi*G*time_interval) = pulse_angle*pi/180;
    Gaussian=Gaussian/sum(Gaussian)*pulse_angle/360/time_interval;
