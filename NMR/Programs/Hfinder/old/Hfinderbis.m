...
function resnorm = Hfinderbis(parameterfile)
...
spins.freqs = 200*(2*rand(1,4)-1);

randguess = 200*(2*rand(1,6)-1);
randguess = randguess - [1250 100 100 700 200 1600];
spins.jfreqs(1,2) = randguess(1); spins.jfreqs(2,1) = randguess(1);
spins.jfreqs(1,3) = randguess(2); spins.jfreqs(3,1) = randguess(2);
spins.jfreqs(1,4) = randguess(3); spins.jfreqs(4,1) = randguess(3);
spins.jfreqs(2,3) = randguess(4); spins.jfreqs(3,2) = randguess(4);
spins.jfreqs(2,4) = randguess(5); spins.jfreqs(4,2) = randguess(5);
spins.jfreqs(3,4) = randguess(6); spins.jfreqs(4,3) = randguess(6);
...
for expct = 1:1:length(params.expnum)
    ...
    %rhobase = rhobase + 0.05*tx{1} +0.05*tx{2};
    ...
    if(params.natabunflag)
        ...
        natabunT2s = [];
        intialampT2 = [initialampT2 params.initialscaling];
    end
    ...
    for l = 1:1:params.loops
       ...
       LB = initialcscoup - [200*ones(1,4)  200*ones(1,length(initialcscoup)-4)];
        ...
        [fittedcscoup,resnorm] = lsqnonlin(@fidelity1,initialcscoup,LB,UB,params.opt,spins,params,rhobase,obsmatbase,expFID,expspec,t,scaling,natabunT2s,gaussianamt,expct);
        initialcscoup = fittedcscoup;
 
        %Do some zero padding and plot
        if(params.plot)
             padding = (1/params.ratio -1)*length(simFID);
        ...
        fittedampT2 = [initialampT2 gaussianamt];
