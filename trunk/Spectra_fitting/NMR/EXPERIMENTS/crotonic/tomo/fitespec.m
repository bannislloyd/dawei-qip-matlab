function spec = fitespec(spec,freq,range,T2,fignum,NN);
%Fit 64 peaks of C1
%
%function spec = fitespec(spec,freq,frange,T2);
%spec spectrum structure 
%freq  frequencies
%frangr range of frequency allowed in the fit
%T2 T2 of the spin
%fignum fignumber to show the fit
...
%  
%  
%  pp1 = round((fr1 + spec.swh/2)*dt*SI);
%  pp2 = round((fr2 + spec.swh/2)*dt*SI);
%  
%  fr = frr(pp1:pp2);
...
[spec.INLP spec.ILP A junk] = fminspleas(funlist,[T2 0],fr,real(spec.spec),[T2-range(1) -range(2)],[T2+range(1) range(2)]);
...
dfe = length(fr)-(2^(NN-1))*2;
V = inv(A'*A) * (junk/dfe);
b = + sqrt(diag(V)) * tinv(1-(1-0.95)/2, dfe);
spec.ILPerr = sqrt(diag(V)) * tinv(1-(1-0.95)/2, dfe);; 
spec.J =A;
spec.sse = junk;
spec.dfe= dfe;
...
function [INLP,ILP,A, junk] = fminspleas(funlist,NLPstart,xdata,ydata,INLB,INUB,weights,options)
